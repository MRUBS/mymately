(function (root, factory) {
    ...
    if (typeof define === 'function' && define.amd) {
        ...
        root.ReserveMembership = factory();
    }
}(this, function () {
    "use strict";

    var ReserveMembership = function (form, options) {
        if (!this || !(this instanceof ReserveMembership)) {
            return new ReserveMembership(form, options);
        }
        ...
        feedback: function (data) {
            if (!data.success) {
                ...
                if (data.errors.email) {
                    ...
                    email.insertAdjacentHTML('beforebegin', error);
                }
            } else {
                var success = this.template(
                    '<div class="cst-success">{report}</div>', {
                    report: data.message
                });

                this.empty(this.form);
                this.form.insertAdjacentHTML('beforebegin', success);
            }
        },
        fail: function (data) {
            console.log(data);
